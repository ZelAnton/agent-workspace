...
- Upgraded to `processkit 0.5` + `vcs-git`/`vcs-jj`/`vcs-github` `0.3`, and migrated the VCS layer onto the published **`vcs-core`** facade. The internal `VcsBackend` trait, `BackendState`, the `Box<dyn>` dispatch, and the hand-rolled backend detection are gone — `crate::vcs::Repo` now wraps a `vcs_core::Repo` and routes each operation to the git/jj helpers via the `repo.git()?` / `repo.jj()?` escape hatches. The CoW worktree flow, op-log rollback, fetch-retry loop, and merge/rebase state machines are unchanged. Transient-fetch classification now uses `vcs_{git,jj}::is_transient_fetch_error` instead of a hand-rolled marker list. No user-visible behavior change.
- Internal: deduplicated the git/jj `processkit::Error` mapping into shared `vcs::error` helpers, and pulled the global+project config merge into a pure, unit-tested `merge_global_project` (with exhaustive per-field coverage so a forgotten merge fails CI). No user-visible behavior change.
...
### Fixed
- Concurrent `ws new` on the same repo no longer risks the copy-on-write path corrupting each other's source repo (a lost stash, the source left on the wrong branch, or a torn reflink tree). The CoW source-mutation now holds a per-repo lock; a second concurrent `ws new` falls back to a plain (concurrency-safe) worktree/workspace add. A crash-leaked lock is reclaimed automatically, and the failure mode is purely speed — never corruption.
- Windows Terminal new-tab integration now strips `;` and control characters from the branch name before it reaches `wt.exe --title`. `wt.exe` splits its command line on `;` even inside quoted args, so a branch like `feat;x` could break the spawn or inject a second tab command — the same hazard the script body already guarded against, now closed for the title too.
- `ws mv` now refuses up front when the destination branch name already exists (instead of relocating the worktree and then failing the rename, leaving it half-renamed), and rescues the shell to the new path if the rename fails after the move.
- A failing `post_merge` hook now states clearly that the merge already SUCCEEDED (so you don't retry and double-merge), and points you at `ws rm` for cleanup.
- The daily update check no longer suppresses itself forever when its timestamp marker is dated in the future (clock skew / restored backup).
