...
`agent-worktree` — a Rust CLI (`wt`) that manages git worktrees for AI coding agents. Distributed as a single static binary, packaged for npm with per-platform subpackages under `npm/`.
...
./scripts/build-npm.sh current        # build + stage binary into npm/agent-worktree-<platform>/bin
...
`src/config/mod.rs` resolves `.agent-worktree.toml` by walking up from `git rev-parse --git-common-dir` — so the same config applies whether the user is in the main repo, a worktree, or a subdirectory.
...
### Hooks run unsandboxed — treat `.agent-worktree.toml` as a committed shell script
...
Worktrees live under `$AGENT_WORKTREE_DIR/workspaces/{repo}-{hash}/` where the hash is derived from the absolute repo path. `git::workspace_id()` is the single source — never construct this path manually. `AGENT_WORKTREE_DIR` defaults to `~/.agent-worktree`; empty string is treated as unset.
