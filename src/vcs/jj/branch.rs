...
pub(super) async fn has_uncommitted_changes(jj: &JjClient, cwd: &Path) -> Result<bool> {
    Ok(!jj.diff_summary(cwd, "@-", "@").await.map_err(map_pk_err)?.is_empty())
}

/// Count of files in the working-copy diff scoped to `path`.
pub(super) async fn uncommitted_count_in(jj: &JjClient, path: &Path) -> Result<usize> {
    Ok(jj.diff_summary(path, "@-", "@").await.map_err(map_pk_err)?.len())
}

/// True iff the working copy has changes OR commits ahead of `trunk`.
pub(super) async fn has_changes_from_trunk(jj: &JjClient, cwd: &Path, trunk: &str) -> Result<bool> {
    if has_uncommitted_changes(jj, cwd).await? {
        return Ok(true);
    }
    Ok(commit_count(jj, cwd, trunk, "@").await? > 0)
}
...
pub(super) async fn is_merge_in_progress(jj: &JjClient, cwd: &Path) -> bool {
...
/// True iff `branch`'s bookmark is reachable from `target`'s ancestors — i.e.
/// branch is fully merged into target.
pub(super) async fn is_merged(jj: &JjClient, cwd: &Path, branch: &str, target: &str) -> Result<bool> {
    let revset = format!("ancestors({target}) & {branch}");
    Ok(commit_count_via_revset(jj, cwd, &revset).await? > 0)
}
...
pub(super) async fn has_diff_from(jj: &JjClient, cwd: &Path, branch: &str, target: &str) -> Result<bool> {
...
pub(super) async fn commit_count(jj: &JjClient, cwd: &Path, from: &str, to: &str) -> Result<usize> {
...
pub(super) async fn commit_count_via_revset(jj: &JjClient, cwd: &Path, revset: &str) -> Result<usize> {
...
pub(super) async fn log_oneline(jj: &JjClient, cwd: &Path, from: &str, to: &str) -> Result<String> {
...
pub(super) async fn diff_shortstat(jj: &JjClient, cwd: &Path, from: &str, to: &str) -> Result<DiffStat> {
...
pub(super) async fn diff_shortstat_in(jj: &JjClient, path: &Path) -> Result<DiffStat> {
